...
def smooth_labels(y_true, label_smoothing, num_classes):
    return y_true * (1. - label_smoothing) + label_smoothing / num_classes
...
def box_ciou(b1, b2):
    ...
    iou = intersect_area / torch.clamp(b1_area + b2_area - intersect_area, min=1e-6)

    # 计算中心差距
    center_dist = torch.sum(torch.pow((b1_xy - b2_xy), 2), axis=-1)
    ...
    ciou = iou - 1. * center_dist / torch.clamp(enclose_diagonal, min=1e-6)
    ...
    def generate(self, train=True, mosaic=True):
        while True:
            ...
            for i in range(len(lines)):
                ...
                inputs.append(np.transpose(img / 255., (2, 0, 1)))
